...
from PyQt5.QtWidgets import QVBoxLayout, QWidget, QPushButton, QGridLayout
...
def ffigure(self, MWindow, position=[100, 100, 500, 500]):
    self.fig = plt.Figure()
    ...
    self.cnvas = FigureCanvas(self.fig)

    self.main_widget = QWidget(self)
    self.setCentralWidget(self.main_widget)

    self.layout = QGridLayout()  # QVBoxLayout()
    self.layout.addWidget(self.cnvas, 1, 0, 5, 1)
    self.main_widget.setLayout(self.layout)

    # matplotlib.interactive(self)

    return(self.fig, self.main_widget, self.cnvas, self.layout)


def aaxes(self, fig, position=[10, 10, 500, 500], ffontsize=13, ffontweight='normal', llinewidth=3, xxlabel='x label', yylabel='y label', ttitle='Title', xxscale='linear', yyscale='linear', xticklbl=None, yticklbl=None, xlim=[-6, 9]):
    ...
    ax.tick_params(width=llinewidth, direction='in', length=7)
...
def bbutton(self, main_widget, position=[10, 10, 30, 15], ffontsize=13, ffontbold=False, Text='Ok',eenable=True):
    ...
    # btn.clicked.connect(btnfunction)
...
def pplot(self, ax1, nnum=1, xxdata=[0], yydata=[0], mmarker='-', pltclr='b', llegend='Legend', legloc=0, leganchor=None, ffontsize=13, ffontweight='normal', llinewidth=3, mmsize=2, legendframe=True, ccolor='blue', xlim=None, yyscale='linear'):
